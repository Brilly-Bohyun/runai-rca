...
- 유사 인시던트: dense RCA-content 매칭으로 찾은 과거의 닮은 인시던트를 family와 workload
  identity로 재랭킹해 표시. 이 인시던트에 RCA가 생기면 질의가 그 RCA를 함께 싣기 때문에
  알림 제목끼리가 아니라 진단끼리 비교합니다. 분석이 나오기 전에는 알림 identity가 전부입니다.
  pod 이름은 양쪽 모두에서 제외합니다 — 재발 때 같은 이름이 다시 나오지 않으므로 잡음만
  더할 뿐입니다.
...
- 인시던트 목록은 최근 활동 순으로 정렬하며, 그 정렬 기준이 되는 시각을 별도의 정렬 가능한
  열로 화면에 노출합니다. 기준을 볼 수 없는 정렬은 운영자에게 임의의 순서로 읽힙니다.
